...
# This script takes a feature (OTU) table and:
# 1. Rarefies the samples to a common depth (removing any samples that fall under a supplied minimum read limit)
# 2. Calculates basic alpha diversity stats for each sample. The calculated stats will be: Richness, Eveness and Hill-numbers
...
option_list <- list(
    make_option(c("-i", "--feature_table"),
        ...
        help = "Path to feature table in TSV format", metavar = "character"
    ),
    make_option(c("-m", "--min_seq_depth"),
        type = "integer", default = 5000,
        help = "Minimum sequencing depth of samples. Samples with fewer reads than this will be removed prior to calculation of diversity statistics (DEFAULT: 5000)", metavar = "character"
    ),
