...
optional arguments:
  ...
  -l, --local_db   Path to local copy of database. Must be in FUNGuild db format [OPTIONAL]
...
python Guilds_v1.1.py -otu user_otu_table.txt -l funguild_db.php
...
Alternatively, a local copy of a funguild database can be supplied using the -l option.
...
# -----------------------------------------------------------------------------
# DADA2-NF local version metadata
# -----------------------------------------------------------------------------
__upstream_name__ = 'FunGuild'
__upstream_version__ = '1.2'
__pipeline_patch__ = 'dada2nf.1'
__version__ = f"{__upstream_version__}+{__pipeline_patch__}"

# Local modifications:
# - Accept local database path via '-l/--local_db' for pipeline-controlled DBs.
# - Keep behavior internal to DADA2-NF (not distributed as standalone release).
...
parser.add_argument("-l", "--local_db", help="Path to a local funguild_db.php file [OPTIONAL]")
...
print(f"{__upstream_name__} v{__version__} (pipeline-internal patch)")
